...
class Cycle_Gan_HD(nn.Module):
    def __init__(self):
        super(Cycle_Gan_HD, self).__init__()
        ...
        # 降采样部分
        # 前几层降采样
        layers['down_layer1'] = base_Conv_bn_lkrl(3, 64, 2)
        
        layers['res_layer'] = Resnet_block(64)
        # 上采样部分
        def upsample_block(in_channels, out_channels, stride):
            return nn.Sequential(
        ...
        # 上采样过程
        layers['up_layer2'] = upsample_block(64, 32, stride=2)

        # 最后一层恢复到1通道
        layers['final_conv'] = nn.Conv2d(32, 1, kernel_size=1)

        self.D_model = nn.Sequential(layers)
        self.init_weight()
...
class Cycle_Gan_LD(nn.Module):
    def __init__(self):
        super(Cycle_Gan_LD, self).__init__()
        ...
        layers['down_layer1'] = base_Conv_bn_lkrl(3, 64, 2)
        # 最后一层不降采样，保持空间维度
        layers['down_layer2'] = base_Conv_bn_lkrl(64, 128, 2)
        layers['down_layer3'] = base_Conv_bn_lkrl(128, 256, 2)
        ...
        layers['res_layer'] = Resnet_block(256)
        
        # 上采样过程
        layers['up_layer1'] = upsample_block(256, 128, stride=2)
        layers['up_layer2'] = upsample_block(128, 64, stride=2)
        layers['up_layer3'] = upsample_block(64, 32, stride=2)
...
if __name__ == '__main__':
    ...
    summary(G, (3, 256, 256))
    HD = Cycle_Gan_HD().to('cuda')
    summary(HD, (3, 256, 256))
    LD = Cycle_Gan_LD().to('cuda')
    summary(LD, (3, 256, 256))